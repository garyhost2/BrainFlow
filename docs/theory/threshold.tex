...
$A\ge\cos\jmath$. It cannot be ruled out --- it \emph{always} happens. The claim
``$\beta^\star$ exists $\iff A<\cos\jmath$'' is false in the direction ``$\Longleftarrow$'' is
used: a transition exists for every admissible $(A,\jmath)$, and $A<\cos\jmath$ is exactly the
condition for that transition to be \emph{unique}. The corrected statement is stronger than the
one it replaces, and it recovers three grid cells the sweep had been discarding.
