...
symmetric coupling; hypotheses (i)--(ii) of Theorem~4.11 are not needed for the response formula.
...
\]
and for $\kappa<1$, with $\mu=2(1-\kappa)$,
...
Corollary~\ref{cor:supp} requires, including off-span directions. (3)~With homogeneous
non-commuting marginals the response to a random symmetric perturbation is $3.5\times10^{-10}$
...
the $\delta\to0$ limit is slope $-1$ (span response linear in $\delta$, skew response $O(1)$),
and the milder measured slope over this window reflects the slow decline of the skew constant
itself with separation ($1.37$ at $\delta=0.05$ to $0.82$ at $\delta=1$). At the ensemble's own
